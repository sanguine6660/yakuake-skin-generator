...
(`app/skin.cpp`, `app/titlebar.cpp`, `app/tabbar.cpp`).
...
**Tab composition** (per `drawTab()`):
`[left corner]` → `[tiled background, width = text + 10px]` → `[right corner]`.
Unselected tabs get separators between them when no unselected corners are set.
...
**Appearance** → select the skin → **Apply**. Restart Yakuake if it doesn't
show up (`killall yakuake && yakuake &`).
...
background, opacity) are **Konsole color schemes** (`.colorscheme` files) —
see `skin_reference/name.colorscheme.reference`.
